...
const ENDPOINT = "https://calcapinew.azurewebsites.net:4001";
...
*/
 ...
 /* const updateVechileStatus = (vchl) => {
      VechileService.update(vchl.id, vchl)
        .then(response => {
  ...
  const updateVechileStatus = (vchl) => {
    ...
    VechileService.updateStatus(vchl.id, vchl.statusId, vchl.lockUser)
      .then(response => {
        console.log(response.data);
        //setMessage("The vechile was updated successfully!");
      })
  ...
  const changeStatus = (vech) => {
    const user = JSON.parse(localStorage.getItem("user"));
    
      if (vech.statusId=="free")  
           {
             vech.statusId = "locked"; 
             vech.lockUser = user.username;
           }
           else if (vech.statusId=="locked")  
           {
             vech.statusId="free";
             vech.lockUser="";
           }  
  ...
  const vechileDialogFooter = (rowdata) => {
    return (
  ...
  const actionBodyTemplate = (rowData) => {
    return (
      <React.Fragment>
        <Button icon="pi pi-lock" className="p-button-rounded p-button-secondary mr-1" 
                style={{background: rowData.statusId=="locked" ? "red":""}} onClick={() => changeStatus(rowData)} />
  ...
  const distTemplate = (rowData) => {
    return <span>{rowData.dist}{rowData.lockUser}</span>;
  }
  ...
  return (
    <div>
      ...
      <DataTable value={vechiles} dataKey="id" responsiveLayout="scroll" selectionMode="single" selection={selectedVechile}
        ...
        <Column header="Distance LockUser" body={distTemplate}></Column>
